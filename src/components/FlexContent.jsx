...
const FlexContent = ({ ifExist, endpoint: {title, heading, text, img, btn, url}}) => {
  return (
    <div>
        <div className='flex items-center justify-between lg:flex-col lg:justify-center nike-container '>
          ...
          ${ifExist ? 'flex-row-reverse' : 'flex-row' } `} >
                ...
                <a href={url} className='flex items-center target={"_blank"} role="button" '>
                    <button type='button' className='button-theme bg-slate-900 text-slate-100 py-1.5 shadow-slate-900'>{btn}</button>
                </a>
            </div>
